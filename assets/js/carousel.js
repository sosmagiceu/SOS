...
export function initCarousels(){
  qsAll("[data-carousel]").forEach(root=>{
    ...
    if(dots){
      dots.innerHTML=slides.map((_,di)=>`<button class="tm-dot${di===0?' is-active':''}" type="button" data-dot="${di}" aria-label="Slide ${di+1}"></button>`).join("");
      dots.addEventListener("click",(e)=>{const b=e.target.closest("[data-dot]"); if(!b) return; set(parseInt(b.dataset.dot,10));});
    }
